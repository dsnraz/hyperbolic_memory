    ...
    **双曲余弦定理**得 O 处内角，夹角越小越相似；`opposite_score` 为该内角（弧度），
    ...
    def __init__(
        ...
        *,
        ...
        enable_cone_gate: bool = True,              # v6_parent_gate
        cone_gate_temperature: float = 0.2,         # v6_parent_gate
        **kwargs: Any,
    ) -> None:
        # v6_parent_gate: 至少启用一种加权（原约束），锥内门控单独作为独立乘子
        if not weight_by_parent_origin_geodesic and not weight_by_parent_anchor_geodesic \
                and not enable_cone_gate:
            raise ValueError(
                "MultiParentAngularHyperbolicRetriever 至少需开启一种父节点加权："
                "weight_by_parent_origin_geodesic / weight_by_parent_anchor_geodesic / "
                "enable_cone_gate (v6_parent_gate)。"
            )
        ...
        self.enable_cone_gate = enable_cone_gate                  # v6_parent_gate
        self.cone_gate_temperature = cone_gate_temperature        # v6_parent_gate
    ...
    def _parent_aggregation_weights(
        ...
        v6_parent_gate: 在原"原点远权重大 / 锚点近权重大"之外新增**锥内门控**：
        query 落在父锥内权重 ≈ 1，在锥外按 exp(-(ratio-1)/τ) 指数衰减，
        ratio = α(父 → query) / half_aperture(父)。
        """
        par = parents_h.detach().float().cpu()
        p_count = par.shape[0]
        ...
        # v6_parent_gate: 锥内门控
        if self.enable_cone_gate:
            query_row = _hyperbolic_spatial_row(query_h).unsqueeze(0).expand(par.shape[0], -1).contiguous()
            # query 对每个父的外角
            alpha_Q = L.cone_vertex_exterior_angle_vectors(par, query_row, curv=curv)
            # 每个父的半孔径
            K = L.half_aperture_vectors(par, curv=curv)
            # ratio < 1 表示 query 在锥内；> 1 表示在锥外
            ratio = alpha_Q / (K + self.weight_eps)
            # 锥内全拿 1，锥外 exp(-(ratio-1)/τ) 衰减
            gate_temp = max(float(self.cone_gate_temperature), 1e-6)
