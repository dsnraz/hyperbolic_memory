    ...
    **双曲余弦定理**得 O 处内角，夹角越小越相似；`opposite_score` 为该内角（弧度），
    ...
    def __init__(
        ...
        *,
        ...
        sim_temperature: float = 0.5,           # v5_inference_sim
        **kwargs: Any,
    ) -> None:
        ...
        self.sim_temperature = sim_temperature    # v5_inference_sim
    ...
    def _weighted_cosine_angular_opposite_score(
        ...
        """
        v5_inference_sim: 改为 sim_i = exp(-α_D / τ)，直接按候选对父的外角打分，
        和训练目标 `-α` 对齐。query 的外角 α_Q 本版不参与 sim（若需利用 α_Q 做
        父的相关性门控，请叠加 v6_parent_gate）。
        """
        curv = self.get_curvature()
        ...
        # v5_inference_sim: 候选的单父相似度 = exp(-α_D / τ)
        tau = max(float(self.sim_temperature), 1e-6)
        sim_i = torch.exp(-an / tau)
