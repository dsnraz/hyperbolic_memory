...
# v9_full_chain: 跨层级对（祖-孙，跨一层）
SKIP_LEVEL_PAIRS = [
    ("DOMAIN", "KEYWORD"),
    ("CATEGORY", "DIALOGUE"),
]

# v9_full_chain: 相邻 + 跨层的全集
ALL_LEVEL_PAIRS = LEVEL_PAIRS + SKIP_LEVEL_PAIRS

# v9_full_chain: 每对"跨层父子"的中间层名称，用于构造祖孙映射
SKIP_LEVEL_MIDDLE = {
    ("DOMAIN", "KEYWORD"):     "CATEGORY",
    ("CATEGORY", "DIALOGUE"):  "KEYWORD",
}
...
class SkipLevelSubtreeSampler(SubtreeSampler):
    ...
    和 SubtreeSampler 的唯一差别：`parent_to_children` 映射的是
    祖父索引 → 孙子索引集合，而非直接父子。

    父子关系通过两跳展开：ancestor.child_ids → middle nodes → middle.child_ids。
    """

    def __init__(
        self,
        ancestor_nodes: List[HierarchicalNode],
        descendant_nodes: List[HierarchicalNode],
        middle_nodes: List[HierarchicalNode],
        parent_level: str,
        child_level: str,
        embedding_dim: int,
        device: torch.device = None,
        ...
        # 提前构造中间层索引（父类 __init__ 会调 _build_index_mappings，
        # 我们在下面 override 它，所以中间层信息必须先挂到 self）
        self._middle_id_to_node = {m.id: m for m in middle_nodes}
    ...
    def _build_index_mappings(self):
        """override: 用两跳展开构造祖孙映射。"""
        self.parent_id_to_idx = {node.id: i for i, node in enumerate(self.parent_nodes)}
        self.child_id_to_idx = {node.id: i for i, node in enumerate(self.child_nodes)}

        self.parent_to_children: Dict[int, List[int]] = {}

        for ancestor_idx, ancestor_node in enumerate(self.parent_nodes):
            grand_ids = set()
            for mid_id in ancestor_node.child_ids:
                mid_node = self._middle_id_to_node.get(mid_id)
                if mid_node:
                    for gid in mid_node.child_ids:
                        if gid in self.child_id_to_idx:
                            grand_ids.add(gid)
            self.parent_to_children[ancestor_idx] = [
                self.child_id_to_idx[gid] for gid in grand_ids
            ]
...
class SubtreeDataset(Dataset):
    ...
    def __init__(
        ...
        use_level_embedding: bool = False,
        level_pair_weights: Optional[Dict[Tuple[str, str], float]] = None,   # v3_mixed_training
    ):
        """
        初始化子树采样数据集。
        ...
        参数:
            ...
            level_pair_weights: v3_mixed_training 新增。键为层级对元组，值为权重。
                        只在 level_pair=None 时生效。传入则按权重随机采样，
                        否则走原有的覆盖式轮转。
        """
        self.embedding_dim = embedding_dim
        self.device = device or torch.device('cpu')
        ...
        self.use_level_embedding = use_level_embedding
        self._level_pair_weights = level_pair_weights    # v3_mixed_training
        ...
        # v9_full_chain: 创建跨层级对采样器（祖孙）
        for ancestor_level, descendant_level in SKIP_LEVEL_PAIRS:
            middle_level = SKIP_LEVEL_MIDDLE[(ancestor_level, descendant_level)]
            ancestor_nodes = nodes_by_level.get(ancestor_level, [])
            descendant_nodes = nodes_by_level.get(descendant_level, [])
            middle_nodes = nodes_by_level.get(middle_level, [])
            if ancestor_nodes and descendant_nodes and middle_nodes:
                skip_sampler = SkipLevelSubtreeSampler(
                ...
                self.samplers[(ancestor_level, descendant_level)] = skip_sampler

        self.available_level_pairs = list(self.samplers.keys())
        self._reset_level_pair_sampling_pool()

        # v3_mixed_training: 统计每层级对被采样次数，训练脚本可读出做健康检查
        self.level_pair_counter: Dict[Tuple[str, str], int] = {p: 0 for p in self.available_level_pairs}
    ...
    def _sample_level_pair_weighted(self) -> Tuple[str, str]:
        """v3_mixed_training: 按权重加权采样层级对。"""
        if not self._level_pair_weights:
            return self._sample_level_pair_with_coverage()
        pairs = list(self.samplers.keys())
        weights = [self._level_pair_weights.get(p, 0.0) for p in pairs]
        total = sum(weights)
        if total <= 0.0:
            # 权重全零退化为均匀覆盖
            return self._sample_level_pair_with_coverage()
    ...
    def __getitem__(self, idx: int) -> SubtreeBatch:
        ...
        if self.level_pair:
            ...
            chosen_pair = self.level_pair
        else:
            # v3_mixed_training: 若传入了权重就走加权采样，否则保持覆盖轮转
            if self._level_pair_weights:
                chosen_pair = self._sample_level_pair_weighted()
            else:
                chosen_pair = self._sample_level_pair_with_coverage()
            sampler = self.samplers[chosen_pair]

        # v3_mixed_training: 记录层级对采样计数
        if chosen_pair in self.level_pair_counter:
            self.level_pair_counter[chosen_pair] += 1
