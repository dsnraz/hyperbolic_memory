...
from model.hyperbolic_utils.hierarchical_dataset import (
    ...
    LEVEL_PAIRS,
    SKIP_LEVEL_PAIRS,                  # v9_full_chain
    ALL_LEVEL_PAIRS,                   # v9_full_chain
    subtree_collate_fn,
)
...
class TrainConfig:
    ...
    # v9_full_chain 新增
    chain_training: bool = False             # 启用时加入跨层级对 (DOMAIN,KEYWORD) 等
    chain_skip_weight: float = 0.5           # 跨层对总共占多少权重（其余给相邻对）
...
class HyperbolicTrainer:
    ...
    def train_mixed(self):
        ...
        v9_full_chain: 若 chain_training=True，加入跨层级对。

        一次性加载全部四层节点，构造一个 level_pair=None 的 SubtreeDataset，
        按 iterations_map 的比例给出权重，每步随机采一个层级对。
        """
        print(f"\n{'='*60}")
        print(f"开始混合批次训练 (v3_mixed_training + v9_full_chain)")
        print(f"{'='*60}")

        total_iter = self.config.mixed_total_iterations or sum(self.config.iterations_map.values())
        print(f"  总步数: {total_iter}")

        # 按原 iterations_map 算相邻层级对权重
        weights_by_idx = self.config.iterations_map
        total_w_adj = sum(weights_by_idx.values())
        adj_weights = {
            LEVEL_PAIRS[i - 1]: weights_by_idx[i] / total_w_adj
            for i in weights_by_idx
        }

        # v9_full_chain: 若启用，把相邻权重按 (1 - chain_skip_weight) 缩放，
        # 跨层对均分 chain_skip_weight
        if self.config.chain_training:
            skip_w_total = float(self.config.chain_skip_weight)
            assert 0 <= skip_w_total < 1, "chain_skip_weight 应在 [0,1)"
            adj_scale = 1.0 - skip_w_total
            weights = {p: w * adj_scale for p, w in adj_weights.items()}
            n_skip = len(SKIP_LEVEL_PAIRS)
            per_skip = skip_w_total / max(n_skip, 1)
            for sp in SKIP_LEVEL_PAIRS:
                weights[sp] = per_skip
        else:
            weights = adj_weights
        ...
        for lp, w in weights.items():
            tag = " (skip)" if lp in SKIP_LEVEL_PAIRS else ""
            print(f"    {lp[0]} -> {lp[1]}: {w:.4f}{tag}")
    ...
    def train(self):
        """执行训练。"""
        # v9_full_chain: chain_training 自动切混合模式
        if self.config.mixed_training or self.config.chain_training:
            self.train_mixed()
...
def parse_args() -> TrainConfig:
    ...
    parser.add_argument('--output_dir', type=str, default='./checkpoints_locomo')
    ...
    # v9_full_chain 新增
    parser.add_argument('--chain_training', action='store_true',
                        help='v9_full_chain: 启用跨层级对训练 (DOMAIN,KEYWORD) 等（自动切混合模式）')
    parser.add_argument('--chain_skip_weight', type=float, default=0.5,
                        help='v9_full_chain: 跨层对总权重，相邻对分享 1-x')
    ...
    return TrainConfig(
        ...
        chain_training=args.chain_training,                    # v9_full_chain
        chain_skip_weight=args.chain_skip_weight,              # v9_full_chain
    )
