...
class HyperbolicTrainer:
    ...
    def _process_batch_feats(self, batch: SubtreeBatch) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        v8_depth_range: projector 保留 2-tuple 返回，但会把最近一次的 target_norm
        和 z_hat_E 的实际模长存到 projector 属性上，loss 里读这两个量做 MSE。
        """
        _, parent_feats_H = self.model(batch.parent_feats)
        batch.parent_target_norm = self.model._last_target_norm      # v8_depth_range
        batch.parent_actual_norm_pre_exp = self.model._last_z_hat_E_norm  # v8_depth_range
        _, child_feats_H  = self.model(batch.child_feats)
        batch.child_target_norm = self.model._last_target_norm       # v8_depth_range
        batch.child_actual_norm_pre_exp = self.model._last_z_hat_E_norm   # v8_depth_range
