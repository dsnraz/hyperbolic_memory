...
class HybridHyperbolicRetriever(GeodesicHyperbolicRetriever):
    ...
    def __init__(
        ...
        *,
        ...
        # v7_domain_geodesic: 默认让 DOMAIN 走测地线，其它层按 _similarity 的新逻辑分派。
        # 配合 _similarity 里 `n_idx > b_idx 走角度` 的反向逻辑：
        #   boundary=DOMAIN → 只有 DOMAIN 走测地，其它走角度。
        self._hybrid_scoring_geodesic_from_level: Optional[HierarchyLevel] = HierarchyLevel.DOMAIN
    ...
    def set_hybrid_scoring_boundary(
        ...
        """
        v7_domain_geodesic: 反向分界语义。
            boundary 以**上**（含）的层走测地，boundary 以**下**的层走角度。
            boundary=None 表示全部走测地。
        """
    ...
    def _rank_nodes(
        ...
        其它类的 ``_rank_nodes``。
        """
        # v7_domain_geodesic: 删除调试打印，保持日志清爽
        scored_hits: List[HyperbolicRetrievalHit] = []
        for node in nodes:
            score, opposite_score = self._similarity(query_h, node)
            if not math.isfinite(opposite_score):
    ...
    def _similarity(
        ...
        """
        v7_domain_geodesic: 反向分界逻辑。
            - boundary 以**上**（含）走测地线：n_idx <= b_idx → geodesic
            - boundary 以**下**走角度：n_idx > b_idx → angular

        举例：
            boundary=DOMAIN    → 只有 DOMAIN 走测地；CATEGORY/KEYWORD/DIALOGUE 走角度
            boundary=CATEGORY  → DOMAIN/CATEGORY 走测地；KEYWORD/DIALOGUE 走角度
            boundary=None      → 全部走测地

        本版默认 boundary=DOMAIN。
        """
