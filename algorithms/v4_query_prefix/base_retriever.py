...
class HierarchicalRetrieverBase(ABC):
    ...
    def __init__(
        ...
        query_prefix: Optional[str] = None,     # v4_query_prefix
    ) -> None:
        self.vector_store = vector_store
        self.embedding_function = embedding_function or vector_store.embedding_function
        self.reranker = reranker
        # v4_query_prefix: 给 query 文本加统一前缀，对齐节点 level_embedding 分布。
        # 推荐值："QUERY: " / "DOMAIN: " / "" 等，先做对比实验再定。
    ...
    def _prepare_query_embedding(
        ...
        # v4_query_prefix: 拼接前缀后再过 embedding_function
        prefixed_text = f"{self.query_prefix}{query_text}" if self.query_prefix else query_text
        return list(self.embedding_function(prefixed_text))
